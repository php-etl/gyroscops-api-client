...
class PipelineCompilationPipelineItem extends \Gyroscops\Api\Runtime\Client\BaseEndpoint implements \Gyroscops\Api\Runtime\Client\Endpoint
{
    ...
    {
        return str_replace(['{id}'], [$this->id], '/runtime/pipelines/{id}/compile');
    }
     ...
     * {@inheritdoc}
     *
     * @return \Gyroscops\Api\Model\PipelineCompilePipelineCommandJsonldRead|null
     *
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (202 === $status) {
            if (mb_strpos($contentType, 'application/ld+json') !== false) {
                return $serializer->deserialize($body, 'Gyroscops\\Api\\Model\\PipelineCompilePipelineCommandJsonldRead', 'json');
            }
        ...
        if (400 === $status) {
            throw new \Gyroscops\Api\Exception\PipelineCompilationPipelineItemBadRequestException($response);
        }
        if (422 === $status) {
            throw new \Gyroscops\Api\Exception\PipelineCompilationPipelineItemUnprocessableEntityException($response);
        }
        if (404 === $status) {
            throw new \Gyroscops\Api\Exception\PipelineCompilationPipelineItemNotFoundException($response);
        }
