...
class AuthenticationOrganizationIdAuthorizationsGetResponse200Normalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return 'Gyroscops\\Api\\Model\\AuthenticationOrganizationIdAuthorizationsGetResponse200' === $type;
    }

    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && 'Gyroscops\\Api\\Model\\AuthenticationOrganizationIdAuthorizationsGetResponse200' === get_class($data);
    }
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        if (\array_key_exists('hydra:view', $data) && null !== $data['hydra:view']) {
            $object->setHydraView($this->denormalizer->denormalize($data['hydra:view'], 'Gyroscops\\Api\\Model\\AuthenticationOrganizationIdAuthorizationsGetResponse200HydraView', 'json', $context));
        ...
        if (\array_key_exists('hydra:search', $data) && null !== $data['hydra:search']) {
            $object->setHydraSearch($this->denormalizer->denormalize($data['hydra:search'], 'Gyroscops\\Api\\Model\\AuthenticationOrganizationIdAuthorizationsGetResponse200HydraSearch', 'json', $context));
